...
use protocol::{Data, LedgerRequest, LedgerResponse, UpdateQuery, Query};
...
impl Call for CoordinatorState {
    async fn new_ledger(&self, request: Request<LedgerRequest>,)
    ...
    async fn append_to_ledger(&self, request: Request<UpdateQuery>,)
        -> Result<Response<protocol::Status>, Status> {
        let UpdateQuery {handle, value} = request.into_inner();
        println!("Received a AppendToLedger Request : {:?} {:?}", handle, value);
    ...
    async fn read_latest(&self, request: Request<Query>,)
        ...
        let value = self.state.read().expect("Failed to acquire read lock")
    ...
    async fn read_at_index(&self, request: Request<Query>,)
                              -> Result<Response<protocol::Response>, Status> {

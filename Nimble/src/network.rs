...
impl EndorserConnection {
    ...
    pub async fn new(uri: String) -> Result<Self, Box<dyn Error>> {
        ...
        let EndorserPublicKey { publickey, signature } = client.get_endorser_public_key(empty_request).await?.into_inner();

        println!("Received PK, Proof from Endorser {:?} = {:?} {:?}", endorser_endpoint, publickey, signature);

        let public_key_instance = PublicKey::from_bytes(&publickey).unwrap();
        let signature_instance = ed25519_dalek::ed25519::signature::Signature::from_bytes(&signature).unwrap();
    ...
    pub async fn call_endorser_new_ledger(&mut self, handle: Vec<u8>)
        -> Result<Signature, Box<dyn Error>>  {
        let request = tonic::Request::new(endorserprotocol::Handle {
            handle
        });
    ...
    pub async fn call_endorser_read_latest(&mut self, handle: Vec<u8>, nonce: Vec<u8>)
        -> Result<Signature, Box<dyn Error>> {
        let request = tonic::Request::new(endorserprotocol::EndorserQuery {
            handle,
            nonce,
        });

        let EndorserQueryResponse {nonce, tail_hash, signature} =

...
use protocol::{Data, LedgerRequest, LedgerResponse, Status, UpdateQuery, Query, Response};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for message in messages {
        ...
        println!("Received Response: {:?} for appending {:?}", status, message.to_vec())
    }
    ...
    let Response { value } = client.read_at_index(index_read_state_query).await?.into_inner();
    let latest_message_data = value.unwrap();
    let latest_message = latest_message_data.content;
